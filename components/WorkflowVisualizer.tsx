...
export function WorkflowVisualizer({ workflowNodes }: { workflowNodes: WorkflowItem[] }) {
  ...
  const selected = workflowNodes[active] || workflowNodes[0];
  ...
  return (
    <div className="workflow-console">
      <div className="workflow-console-bar">
        ...
        <div className="workflow-console-meta">
          <span>6 CONNECTED NODES</span>
      ...
      <div className="workflow-console-body">
            ...
            {workflowNodes.map((node, index) => {
              ...
              return (
                <div className="workflow-step-wrap" key={node.name}>
                  <div className="workflow-step-card-holder">
                    <PixelCard
                      as="button"
                      variant="glass"
                      gridSize={6}
                      onClick={() => setActive(index)}
                      className={`workflow-step ${active === index ? "is-active" : ""}`}
                  ...
                  {index < workflowNodes.length - 1 && (
                    <div
                      className={`workflow-connector ${index < active ? "is-complete" : ""} ${
                        index === active ? "is-flowing" : ""
                      }`}
                      aria-hidden="true"
                    >
        ...
        <aside className="workflow-inspector">
          <div className="workflow-inspector-top">
            <span>NODE / {String(active + 1).padStart(2, "0")}</span>
            <i>ACTIVE</i>
          </div>
          <div className="workflow-inspector-icon">
            {(() => {
              const SelectedIcon = icons[active % icons.length];
              return <SelectedIcon size={25} />;
            })()}
          </div>
          <h3>{selected.name}</h3>
          <p>{selected.desc}</p>
