...
import { Activity, ArrowDown, ArrowLeft, Layers3 } from "lucide-react";
...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const projectDetail = fallbackDetail
    ? {
        ...fallbackDetail,
        problem: project.problem || fallbackDetail.problem,
        solution: project.solution || fallbackDetail.solution,
        nodes: project.nodes?.length ? project.nodes : fallbackDetail.nodes,
        capabilities: project.capabilities?.length ? project.capabilities : fallbackDetail.capabilities,
        implementation: project.implementation || fallbackDetail.implementation,
        stats: project.stats?.length ? project.stats : fallbackDetail.stats,
      }
    : project.problem &&
      project.solution &&
    ...
    <main className="case-page">
      <header className="case-nav" style={{ backdropFilter: "none", WebkitBackdropFilter: "none" }}>
        ...
        <div className="shell case-nav-inner">
          ...
          <Link href="/" className="case-brand" aria-label="Mehedi portfolio home">
            mehedi<span>.</span>
          </Link>
        ...
        <div className="shell case-hero-layout">
          <div className="case-hero-copy">
            <div className="case-kicker">
              <span>
                <PixelRadio size={11} /> DEPLOYMENT {project.number}
              </span>
              {project.client}
            </div>
            <h1>{project.title}</h1>
          ...
          <aside className="case-hero-system" aria-label="System overview">
            ...
            <div className="case-system-list">
              {projectDetail.nodes.map((node, index) => (
                <div key={node}>
            ...
            <div className="case-system-foot">
              <Activity size={14} />
              <span>Architecture designed for observable operation</span>
            </div>
          </aside>
        </div>
        <div className="shell case-stat-strip">
          {projectDetail.stats.map(([value, label], index) => (
            <div key={label}>
      ...
      <section className="case-story" id="case-story">
        <div className="shell">
          ...
          <div className="case-story-grid">
            ...
            <div className="case-story-arrow" aria-hidden="true">
      ...
      <section className="case-architecture-section">
        <div className="shell">
          <div className="case-section-intro case-section-intro-row">
            <div>
              <span>02 / ARCHITECTURE</span>
              <h2>
                Open the system.
                <br />
                <em>Inspect every layer.</em>
              </h2>
            </div>
            <p>Select a component to see how responsibility stays clear across the wider automation.</p>
          </div>
        ...
        <div className="shell case-build-layout">
          <div className="case-build-copy">
            ...
            <h2>
              Built to operate,
              <br />
              not just demonstrate.
            </h2>
            <p>{projectDetail.implementation}</p>
            <div className="case-build-principle">
              <Layers3 size={18} />
              <div>
                <strong>Modular by design</strong>
                <span>Components can be maintained, replaced and scaled without rebuilding the entire system.</span>
              </div>
            </div>
          </div>
          <aside className="case-capabilities">
            <div className="case-capabilities-head">
              <PixelSparkles size={17} />
              <span>SYSTEM CAPABILITIES</span>
              <i>{String(projectDetail.capabilities.length).padStart(2, "0")}</i>
            </div>
            <div className="case-capabilities-list">
              {projectDetail.capabilities.map((capability, index) => (
                <div key={capability}>
                  <span>{String(index + 1).padStart(2, "0")}</span>
      ...
      <section className="case-cta">
        <div className="shell case-cta-layout">
          <div>
            ...
            <h2>
              Let’s turn the messy process
              <br />
              into something that runs.
            </h2>
            <PixelCard as="a" href="/#contact" variant="primaryButton" gridSize={6} className="case-cta-primary">
              Start a project <PixelArrowRight size={16} />
            </PixelCard>
          </div>
          <PixelCard
            as="a"
            ...
            className="case-next"
          >
            <small>NEXT CASE STUDY / {nextProject.number}</small>
